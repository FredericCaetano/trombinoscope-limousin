...
import { Search, Plus, Pencil, Trash2, X, Phone, Mail, ShieldCheck, Users, LayoutGrid, Upload, MapPin, Settings2, Camera, Printer, Check, FileText } from "lucide-react";
...
/* ---------------------------------------------------------------
   STOCKAGE PARTAGÉ — toutes les modifications sont visibles par tous
   Les données sont sauvegardées en mode édition et chargées au démarrage
--------------------------------------------------------------- */
const SK = {
  data:  "trombi:data",
  photo: (id) => `trombi:photo:${id}`,
};

async function chargerDonnees() {
  try {
    const r = await window.storage.get(SK.data, true);
    if (!r) return null;
    const data = JSON.parse(r.value);
    // Charger les photos séparément (stockées par personne)
    const personnesAvecPhotos = await Promise.all(
      data.personnes.map(async (p) => {
        try {
          const pr = await window.storage.get(SK.photo(p.id), true);
          return pr ? { ...p, photoUrl: pr.value } : p;
        } catch { return p; }
      })
    );
    return { ...data, personnes: personnesAvecPhotos };
  } catch (e) {
...
async function sauvegarderDonnees(personnes, qualifItems, orderedItemIds, pq) {
  try {
    // Données principales sans les photos (trop lourdes pour un seul bloc)
    const personnesSansPhotos = personnes.map((p) => ({ ...p, photoUrl: null }));
    await window.storage.set(SK.data, JSON.stringify({
      personnes: personnesSansPhotos,
      items: qualifItems,
    ...
    for (const p of personnes) {
      if (p.photoUrl?.startsWith("data:")) {
        await window.storage.set(SK.photo(p.id), p.photoUrl, true);
      }
    }
  } catch (e) {
...
export default function App() {
  ...
  const [personnes, setPersonnes]       = useState(INITIAL_PERSONNES);
  const [pq, setPq]                     = useState(INITIAL_PQ);
  const [qualifItems, setQualifItems]   = useState(INITIAL_QUALIF_ITEMS);
  const [orderedItemIds, setOrderedItemIds] = useState(INITIAL_ORDERED_IDS);
  ...
  useEffect(() => {
    chargerDonnees().then((data) => {
      if (data) {
        ...
        setQualifItems(data.items);
        setOrderedItemIds(data.order);
        setPq(data.pq);
      }
      setLoading(false);
    });
  ...
  // ── Auto-sauvegarde debounced dès qu'une donnée change en mode édition ──
  useEffect(() => {
    if (!adminMode || loading) return;
    setSaving(true);
    const t = setTimeout(async () => {
      await sauvegarderDonnees(personnes, qualifItems, orderedItemIds, pq);
      setLastSaved(new Date());
      setSaving(false);
    }, 800);
    return () => clearTimeout(t);
  }, [personnes, qualifItems, orderedItemIds, pq, adminMode, loading]);
  ...
  function savePersonne(data) {
    if (data.id) {
      setPersonnes((prev) => prev.map((p) => (p.id === data.id ? data : p)));
    } else {
      const id = `p${Date.now()}`;
      setPersonnes((prev) => [...prev, { ...data, id }]);
      setPq((prev) => ({ ...prev, [id]: [] }));
    }
  ...
  function updatePersonPhoto(id, photoUrl) {
    setPersonnes((prev) => prev.map((p) => (p.id === id ? { ...p, photoUrl } : p)));
  }

  function toggleItem(personneId, itemId) {
    setPq((prev) => {
  ...
  function addQualifItem(catLabel, numero, libelle, color) {
    const id = `q${Date.now()}`;
    setQualifItems((prev) => [...prev, { id, cat: "custom", catLabel, numero, libelle, color }]);
    setOrderedItemIds((prev) => [...prev, id]);
    setAddingItem(false);
  }

  function deleteQualifItem(itemId) {
  ...
  function changeItemColor(itemId, color) {
    setQualifItems((prev) => prev.map((it) => (it.id === itemId ? { ...it, color } : it)));
    setColorPickerFor(null);
  }

  // Déplacer un item avant un autre (drag-and-drop ligne)
  ...
  // Déplacer une catégorie (couleur) avant une autre (drag-and-drop groupe)
  function moveCatBefore(dragColor, targetColor) {
