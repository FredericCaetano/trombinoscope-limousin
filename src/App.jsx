...
/* ---------------------------------------------------------------
   HELPERS SUPABASE — photos dans Storage, données dans les tables
--------------------------------------------------------------- */
...
export default function App() {
  ...
  // ── Helpers pour reconstruire l'état depuis les données brutes Supabase ──
  ...
  // ── Temps réel : écoute les changements de tous les autres utilisateurs ──
  useEffect(() => {
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'qualif_items' }, async () => {
        ...
        setQualifItems(data || []);
        setOrderedItemIds((data || []).map((it) => it.id));
      })
  ...
  async function savePersonne(data) {
    ...
    if (data.id && personnes.find((p) => p.id === data.id)) {
      ...
      const { photoUrl, ...rest } = { ...data, id };
      await supabase.from('personnes').insert(rest);
    }
    markSaved();
  ...
  async function updatePersonPhoto(id, base64DataUrl) {
    ...
    try {
      ...
      setPersonnes((prev) => prev.map((p) => (p.id === id ? { ...p, photo_url: photoUrl } : p)));
    } catch (e) {
  ...
  async function toggleItem(personneId, itemId) {
    ...
    setSaving(true);
    if (held) {
      await supabase.from('personne_qualifications').delete()
        .match({ personne_id: personneId, item_id: itemId });
    } else {
      await supabase.from('personne_qualifications').insert(
        { personne_id: personneId, item_id: itemId }
      );
    }
    markSaved();
  }
  ...
  async function addQualifItem(catLabel, numero, libelle, color) {
    ...
    setSaving(true);
    await supabase.from('qualif_items').insert(
      { id, cat: 'custom', cat_label: catLabel, numero, libelle, color, position }
    );
    markSaved();
    setAddingItem(false);
  }
  ...
  async function moveItemBefore(dragId, targetId) {
    ...
    const filtered = newOrder.filter((id) => id !== dragId);
    const idx = filtered.indexOf(targetId);
    filtered.splice(idx < 0 ? filtered.length : idx, 0, dragId);
    setOrderedItemIds(filtered);
    setSaving(true);
    await Promise.all(
      filtered.map((id, pos) =>
        supabase.from('qualif_items').update({ position: pos }).eq('id', id)
      )
    );
    markSaved();
  }
    ...
    <div className="min-h-screen bg-slate-100 text-slate-900 font-sans p-3 sm:p-6 print:p-0">
      <style>{`
        @media print {
          ...
          .impression-page { page-break-after: always; }
      ...
      {/* HEADER */}
      <div className="no-print max-w-[1320px] mx-auto px-6">
        <header
        ...
        <div className="px-6 pt-5 pb-4">
          ...
          <div className="flex items-center gap-2 mt-5">
            ...
            <TabButton active={view === "qualifs"} onClick={() => setView("qualifs")} icon={<ShieldCheck size={16} />} label="Qualifications" />
            <TabButton active={view === "impression"} onClick={() => setView("impression")} icon={<FileText size={16} />} label="Impression A3" />
          </div>
        </div>
      ...
      {/* CONTENU */}
      <main className={`mx-auto px-6 py-8 print:max-w-none print:px-0 print:py-0 ${view === "impression" ? "max-w-none" : "max-w-[1320px]"}`}>
        {view === "trombi" ? (
          <TrombinoscopeView
      ...
      <footer className="no-print max-w-[1320px] mx-auto px-6 pb-10 text-xs text-slate-400">
...
function Avatar({ personne, size = 64 }) {
  if (personne.photo_url || personne.photoUrl) {
    return (
      <img
        src={personne.photo_url || personne.photoUrl}
...
--------------------------------------------------------------- */
function ImpressionView({ agences, personnes, items, pq, orderedItemIds }) {
  ...
  const printAgences = agences.filter((a) => a.id !== "direction");
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 32 }}>
      ...
      <div className="no-print" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        <div>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: "#1e293b" }}>Aperçu impression — Format A3 paysage</h2>
          <p style={{ margin: "4px 0 0", fontSize: 13, color: "#94a3b8" }}>
            Une page A3 par agence · cellules colorées = mission acquise · texte du code de qualification dans chaque cellule
          </p>
        </div>
        <button
          onClick={() => window.print()}
      ...
      {printAgences.map((agence) => {
        ...
        return (
          <div
            ...
            style={{ background: "white", borderRadius: 16, border: "1px solid #e2e8f0", overflow: "hidden", fontFamily: "'Segoe UI', system-ui, sans-serif" }}
          >
            {/* En-tête */}
            <div style={{ display: "flex", alignItems: "center", gap: 16, padding: "10px 16px", background: "linear-gradient(135deg,#1e3a5f,#2563eb)" }}>
            ...
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "8px" }}>
                ...
                <thead>
                  <tr>
                    {/* Col code catégorie */}
                    <th style={{ width: 52, minWidth: 52, background: "#f8fafc", borderBottom: "2px solid #e2e8f0", borderRight: "2px solid #cbd5e1", padding: "4px 2px", fontSize: "7px", color: "#94a3b8", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>Code</th>
                    {/* Col intitulé mission */}
                  ...
                  {groups.map((group) =>
                    group.items.map((item, itemIdx) => (
                      <tr key={item.id} style={{ borderBottom: itemIdx === group.items.length - 1 ? "2px solid " + group.color + "50" : "1px solid #f1f5f9" }}>
                        {/* Cellule code catégorie (fusionnée verticalement) */}
                        {itemIdx === 0 && (
                          <td
                        ...
                        {/* Intitulé mission */}
                        <td style={{ padding: "3px 10px", borderRight: "2px solid #e2e8f0", color: "#334155", fontSize: "9px", whiteSpace: "nowrap", background: "#fafafa", height: 22 }}>
                          <span style={{ color: "#94a3b8", marginRight: 4, fontVariantNumeric: "tabular-nums", fontSize: "8.5px" }}>{item.numero}</span>
                          {item.libelle}
                        </td>
                        ...
                        {liste.map((p) => {
                          ...
                          return (
                            <td key={p.id} style={{ borderRight: "1px solid #e2e8f0", padding: 0, textAlign: "center", verticalAlign: "middle", height: 22 }}>
                              {held ? (
